...
Round Robin Notes uses a **single-provider AI architecture**:

1. **Primary**: OpenAI GPT-4 models (GPT-4o, GPT-4o-mini) for all clinical AI features
...
| Function | File | Primary Provider | Model | Purpose |
|----------|------|-----------------|-------|---------|
| `ai-clinical-assistant` | `supabase/functions/ai-clinical-assistant/index.ts` | OpenAI | GPT-4o / GPT-4o-mini | 7 clinical AI features |
| `transcribe-audio` | `supabase/functions/transcribe-audio/index.ts` | OpenAI | Whisper-1 + GPT-4o-mini | Audio → text transcription |
| `transform-text` | `supabase/functions/transform-text/index.ts` | OpenAI | GPT-4o-mini | Text formatting |
| `generate-daily-summary` | `supabase/functions/generate-daily-summary/index.ts` | OpenAI | GPT-4o-mini | Daily summaries |
| `generate-patient-course` | `supabase/functions/generate-patient-course/index.ts` | OpenAI | GPT-4o-mini | Hospital course narratives |
| `generate-interval-events` | `supabase/functions/generate-interval-events/index.ts` | OpenAI | GPT-4o | Interval event summaries |
| `format-medications` | `supabase/functions/format-medications/index.ts` | OpenAI | GPT-4o-mini | Medication categorization |
| `generate-todos` | `supabase/functions/generate-todos/index.ts` | OpenAI | GPT-4o-mini | Todo generation |
| `parse-single-patient` | `supabase/functions/parse-single-patient/index.ts` | OpenAI | GPT-4o | Clinical note parsing |
| `parse-handoff` | `supabase/functions/parse-handoff/index.ts` | OpenAI | GPT-4o-mini | Epic handoff parsing |
...
3. **Whisper has zero fallback**: If OpenAI is unavailable, transcription completely fails.
4. **No retry logic**: Beyond Supabase's built-in retry, there is no application-level retry with exponential backoff.
5. **No response caching**: Identical requests always hit the API.

## Reliability Risks

1. **Single point of failure**: All functions depend entirely on OpenAI API availability.
