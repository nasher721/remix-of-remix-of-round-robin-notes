...
export interface Apache2Inputs {
  ...
  paco2?: number; // PaCO2 (alternative to resp rate)
  pao2?: number; // PaO2
  ...
  hematocrit?: number;
  wbc?: number; // White blood cell count x10^3/uL
  chronicHealth?: {
    severeOrganInsufficiency?: boolean;
    immunocompromised?: boolean;
    postoperative?: boolean; // admitted postoperatively
  };
...
export const calculateApache2 = (inputs: Apache2Inputs): RiskScoreResult => {
  let acutePhysiologyScore = 0;
  let agePoints = 0;
  let chronicHealthPoints = 0;

  // Age points
  if (inputs.age !== undefined) {
    if (inputs.age >= 75) agePoints = 6;
    else if (inputs.age >= 65) agePoints = 5;
    else if (inputs.age >= 55) agePoints = 3;
    else if (inputs.age >= 45) agePoints = 2;
    else agePoints = 0;
  }

  // Chronic health points
  const ch = inputs.chronicHealth;
  if (ch?.severeOrganInsufficiency || ch?.immunocompromised) {
    chronicHealthPoints = ch?.postoperative ? 2 : 5;
  }

  // Temperature
  if (inputs.temperature !== undefined) {
    if (inputs.temperature >= 41) acutePhysiologyScore += 4;
    else if (inputs.temperature >= 39) acutePhysiologyScore += 3;
    else if (inputs.temperature >= 38.5) acutePhysiologyScore += 1;
    else if (inputs.temperature >= 36) acutePhysiologyScore += 0;
    else if (inputs.temperature >= 34) acutePhysiologyScore += 1;
    else if (inputs.temperature >= 32) acutePhysiologyScore += 2;
    else if (inputs.temperature >= 30) acutePhysiologyScore += 3;
    else acutePhysiologyScore += 4;
  }

  // Mean arterial pressure
  if (inputs.map !== undefined) {
    if (inputs.map >= 160) acutePhysiologyScore += 4;
    else if (inputs.map >= 130) acutePhysiologyScore += 3;
    else if (inputs.map >= 110) acutePhysiologyScore += 2;
    else if (inputs.map >= 70) acutePhysiologyScore += 0;
    else if (inputs.map >= 50) acutePhysiologyScore += 2;
    else acutePhysiologyScore += 4;
  }

  // Heart rate
  if (inputs.heartRate !== undefined) {
    if (inputs.heartRate >= 180) acutePhysiologyScore += 4;
    else if (inputs.heartRate >= 140) acutePhysiologyScore += 3;
    else if (inputs.heartRate >= 110) acutePhysiologyScore += 2;
    else if (inputs.heartRate >= 70) acutePhysiologyScore += 0;
    else if (inputs.heartRate >= 55) acutePhysiologyScore += 2;
    else if (inputs.heartRate >= 40) acutePhysiologyScore += 3;
    else acutePhysiologyScore += 4;
  }

  // Respiratory rate / PaCO2
  if (inputs.respiratoryRate !== undefined || inputs.paco2 !== undefined) {
    const rr = inputs.respiratoryRate;
    const paco2 = inputs.paco2;
    if (rr !== undefined) {
      if (rr >= 50) acutePhysiologyScore += 4;
      else if (rr >= 35) acutePhysiologyScore += 3;
      else if (rr < 12) acutePhysiologyScore += 2;
      else if (rr >= 12 && rr <= 24) acutePhysiologyScore += 0;
      else if (rr >= 25 && rr <= 34) acutePhysiologyScore += 0;
    }
    if (paco2 !== undefined) {
      if (paco2 < 25) acutePhysiologyScore += 4;
      else if (paco2 >= 25 && paco2 <= 34) acutePhysiologyScore += 3;
      else if (paco2 >= 35 && paco2 <= 49) acutePhysiologyScore += 2;
      else if (paco2 >= 50 && paco2 <= 70) acutePhysiologyScore += 0;
    }
  }

  // PaO2 (adjusted for FiO2)
  if (inputs.pao2 !== undefined && inputs.fio2 !== undefined) {
    const pao2 = inputs.pao2;
    const fio2 = inputs.fio2;

    if (fio2 >= 0.5) {
      if (pao2 < 55) acutePhysiologyScore += 4;
      else if (pao2 < 61) acutePhysiologyScore += 3;
      else if (pao2 < 71) acutePhysiologyScore += 2;
      else acutePhysiologyScore += 0;
    } else {
      if (pao2 < 55) acutePhysiologyScore += 4;
      else if (pao2 < 61) acutePhysiologyScore += 3;
      else if (pao2 < 71) acutePhysiologyScore += 2;
  ...
  if (inputs.arterialPh !== undefined) {
    const ph = inputs.arterialPh;
    if (ph >= 7.7) acutePhysiologyScore += 4;
    else if (ph >= 7.6) acutePhysiologyScore += 3;
    else if (ph >= 7.5) acutePhysiologyScore += 1;
    else if (ph >= 7.33 && ph <= 7.49) acutePhysiologyScore += 0;
    else if (ph >= 7.25 && ph < 7.33) acutePhysiologyScore += 2;
    else if (ph >= 7.15 && ph < 7.25) acutePhysiologyScore += 3;
    else if (ph < 7.15) acutePhysiologyScore += 4;
  }

  // Serum sodium
  if (inputs.serumSodium !== undefined) {
    const na = inputs.serumSodium;
    if (na >= 180) acutePhysiologyScore += 4;
    else if (na >= 160) acutePhysiologyScore += 3;
    else if (na >= 155) acutePhysiologyScore += 2;
    else if (na >= 150) acutePhysiologyScore += 1;
    else if (na >= 130 && na <= 149) acutePhysiologyScore += 0;
    else if (na >= 120 && na < 130) acutePhysiologyScore += 2;
    else if (na >= 111 && na < 120) acutePhysiologyScore += 3;
    else acutePhysiologyScore += 4;
  }

  // Serum potassium
  if (inputs.serumPotassium !== undefined) {
    const k = inputs.serumPotassium;
    if (k >= 7) acutePhysiologyScore += 4;
    else if (k >= 6) acutePhysiologyScore += 3;
    else if (k >= 5.5) acutePhysiologyScore += 1;
    else if (k >= 3.5 && k <= 5.4) acutePhysiologyScore += 0;
    else if (k >= 3 && k < 3.5) acutePhysiologyScore += 1;
    else if (k >= 2.5 && k < 3) acutePhysiologyScore += 2;
    else acutePhysiologyScore += 4;
  }
  ...
  if (inputs.creatinine !== undefined) {
    ...
    if (inputs.acuteRenalFailure) {
      if (cr >= 3.5) acutePhysiologyScore += 4;
      else if (cr >= 2) acutePhysiologyScore += 3;
    } else {
      if (cr >= 1.6 && cr <= 1.9) acutePhysiologyScore += 2;
      else if (cr >= 1.4 && cr <= 1.5) acutePhysiologyScore += 1;
      else if (cr >= 0.6 && cr <= 1.3) acutePhysiologyScore += 0;
      else if (cr < 0.6) acutePhysiologyScore += 2;
    }
  }

  // Hematocrit
  if (inputs.hematocrit !== undefined) {
    const hct = inputs.hematocrit;
    if (hct >= 60) acutePhysiologyScore += 4;
    else if (hct >= 50) acutePhysiologyScore += 2;
    else if (hct >= 46) acutePhysiologyScore += 1;
    else if (hct >= 30 && hct < 46) acutePhysiologyScore += 0;
    else if (hct >= 20 && hct < 30) acutePhysiologyScore += 2;
    else acutePhysiologyScore += 4;
  }
  ...
  if (inputs.wbc !== undefined) {
    ...
    if (wbc >= 40) acutePhysiologyScore += 4;
    else if (wbc >= 20) acutePhysiologyScore += 2;
    else if (wbc >= 15) acutePhysiologyScore += 1;
    else if (wbc >= 3 && wbc < 15) acutePhysiologyScore += 0;
    else if (wbc >= 1 && wbc < 3) acutePhysiologyScore += 2;
    else acutePhysiologyScore += 4;
  }

  // Glasgow Coma Scale (deducted from 15 - GCS)
  if (inputs.glasgowComaScale !== undefined) {
    acutePhysiologyScore += (15 - inputs.glasgowComaScale);
  }

  const totalScore = acutePhysiologyScore + agePoints + chronicHealthPoints;
  const maxScore = 71;

  const riskLevel = totalScore <= 9 ? 'low' :
    totalScore <= 19 ? 'moderate' :
    totalScore <= 34 ? 'high' : 'critical';

  const mortality = totalScore <= 9 ? '<10%' :
    totalScore <= 19 ? '10-20%' :
    totalScore <= 34 ? '20-50%' :
    '50-100%';
  ...
  return {
    score: totalScore,
    maxScore,
    interpretation: `APS: ${acutePhysiologyScore} | Age: ${agePoints} | Chronic: ${chronicHealthPoints}`,
    riskLevel,
    recommendation: `Estimated mortality: ${mortality}`,
    color: riskLevel === 'low' ? 'text-green-600' :
